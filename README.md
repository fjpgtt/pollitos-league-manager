...
In each league there will be team, player and matches.
For the team we need one ID, name and player.
For the player we need an ID, name.
For the matches, it will have both team and the score of each one

We would need two have 2 profiles: The default one that will be empty and the populated,
the populated will have at least 2 team each one with 2 player already created in each league.

There will be a limit of how many team will be in a league, by the default it will be 10 but you can control that with a property
